  ...
  while (i < words.length) {
    ...
    if (match) {
      ...
      if (underlineNextWord) {
        ...
        underlineNextWord = false; // reset so only one word is underlined
      }
      ...
      if (/^@?100%?$/.test(cleanWord)) {
        ...
        underlineNextWord = true; // next word should be underlined (if it's matched)
        i++;
      } else {
        result.push(words[i]);
        i++;
