...
function formatUnits(actionText) {
  // Combine multi-word and single-word units
  const combinedUnits = [
    ...["drop overlord"], // Multi-word units (prioritized)
    ...units.zerg,
    ...units.protoss,
    ...units.terran,
  ];

  // Iterate through the combined units and apply formatting
  combinedUnits.forEach((unit) => {
    const isMultiWord = unit.includes(" ");
    const regex = isMultiWord
      ? new RegExp(`\\b${unit}\\b`, "gi") // Exact match for multi-word units
      : new RegExp(
          `\\b${unit}(s)?\\b(?!\\s+warren|\\s+den|\\s+pit|\\s+network|\\s+speed|\\s+armor|\\s+nest|\\s+core|\\s+shrine|\\s+gate|\\s+forge|\\s+range|\\s+barracks|\\s+command|\\s+reactor|\\s+academy)`,
          "gi"
        ); // Single-word units with context exclusions

    const colorClass = units.zerg.includes(unit)
      ? "bold-purple"
      : units.protoss.includes(unit)
      ? "bold-blue"
      : units.terran.includes(unit)
      ? "bold-red"
      : "";
    ...
    actionText = actionText.replace(regex, (match, plural) => {
      ...
      const imageTag = imageSrc
        ...
        : "";
      return `<span class="${colorClass}">${capitalizeFirstLetter(unit)}${
        plural || ""
      }${imageTag}</span>`;
    });
  });
...
export function formatActionText(actionText) {
  // Handle specific multi-word terms
  const multiWordTerms = [
    {
      regex: /\bgreater spire\b/gi,
      replacement: `<span class="bold-yellow">Greater Spire <img src="img/structure/greater_spire.png" alt="Greater Spire" class="structure-image"></span>`,
    },
    {
      regex: /\bcaduceus reactor\b/gi,
      replacement: `<span class="upgrade-highlight">Caduceus Reactor <img src="img/upgrade/caduceus_reactor.png" alt="Caduceus Reactor" class="upgrade-image"></span>`,
    },
    {
      regex: /\bresearch warp gate\b/gi,
      replacement: `<span class="upgrade-highlight">Research Warp Gate <img src="img/upgrade/warp_gate.png" alt="Research Warp Gate" class="upgrade-image"></span>`,
    },
  ];

  // Replace multi-word terms first
  multiWordTerms.forEach((term) => {
    actionText = actionText.replace(term.regex, term.replacement);
  });

  // Format abbreviations, units, structures, upgrades, etc.
